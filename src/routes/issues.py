...
async def create_issue(payload: CreateIssue, response: Response):
    """
    Create an issue in your configured GitHub repo.
    Returns 201 + Location header on success.
    """
    if not payload.title or not payload.title.strip():
        raise HTTPException(status_code=400, detail={"error": "BadRequest", "message": "title is required"})
    try:
        ...
        if e.status in (401, 403):
            raise HTTPException(status_code=401, detail={"error": "Unauthorized", "message": e.message, "details": e.details})
        elif e.status == 404:
            raise HTTPException(status_code=404, detail={"error": "NotFound", "message": "Repo not found or no access", "details": e.details})
        else:
            raise HTTPException(status_code=502, detail={"error": "GitHubError", "message": e.message, "details": e.details})

@router.get("/issues", response_model=List[Issue])
async def list_issues(
    response: Response,
    state: str = Query("open", pattern="^(open|closed|all)$"),
    labels: Optional[str] = Query(None, description='Comma-separated labels, e.g. "bug,frontend"'),
    page: int = Query(1, ge=1),
    per_page: int = Query(30, ge=1, le=100),
):
    """
    List issues with GitHub-like pagination params.
    Also forwards GitHub's Link and rate limit headers.
    """
    try:
        ...
        raise HTTPException(status_code=code, detail={"error": err, "message": e.message, "details": e.details})
...
async def get_issue(number: int = Path(..., ge=1)):
    ...
    try:
        ...
        if e.status == 404:
            raise HTTPException(status_code=404, detail={"error": "NotFound", "message": f"Issue {number} not found", "details": e.details})
...
async def patch_issue(
    ...
    """
    Update an issue: change title/body or open/close it (state: 'open' or 'closed').
    """
    try:
...
async def add_comment(number: int, payload: CreateComment):
    ...
    if not payload.body or not payload.body.strip():
        raise HTTPException(status_code=400, detail={"error": "BadRequest", "message": "comment body is required"})
    try:
